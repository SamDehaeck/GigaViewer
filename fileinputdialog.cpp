...
FileInputDialog::FileInputDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::FileInputDialog),parent(parent)
{
    ui->setupUi(this);
...
{
    QString resp = QFileDialog::getOpenFileName(parent,tr("Open Picture"),
...
{
    QString resp = QFileDialog::getOpenFileName(parent,tr("Open Movie"),
    currentDir.absolutePath(), tr("All files (*.*)") );
    if (resp=="") {
        currentDir=QDir::home();
    } else {
